...
class MultiStepSelection extends MultiStepDisplay {
  ...
  public static function handleAjaxCommand(array $form, FormStateInterface $form_state) {
    ...
    $message = \Drupal::translation()->translate('Only %cardinality items can be used.', ['%cardinality' => $cardinality])->render();
    if ($cardinality == 1) {
      $message = \Drupal::translation()->translate('Only %cardinality item can be used.', ['%cardinality' => $cardinality])->render();
    }
